#' ---
#' title: "Reproducing dRiftDM Article Output - package version 0.3.0"
...
prms_solve(a_model)["dx"] <- .005 # changes the space discretization
...
#+ set_data
# create a model
a_model <- dmc_dm(var_non_dec = FALSE, var_start = TRUE) # creates the Diffusion Model for Conflict Tasks
...
prms_solve(a_model)["dt"] <- .005
prms_solve(a_model)["dx"] <- .005

# create the search space
l_u = get_lower_upper(a_model)
# -> this way, the lower and upper search
