...
complexity (Vandekerckhove et al. 2015), and summarize the results in a
contingency table—also known as a confusion matrix (see Wilson and
Collins 2019).
...
(see, e.g., Vandekerckhove et al. 2015).
...
$`p(\text{fit model} \mid \text{sim model})`$).
...
matrix quantifies $`p(\text{fit model} \mid \text{sim model})`$, the
inversion matrix quantifies
$`p(\text{sim model} \mid \text{fit model})`$—that is, the probability
that data best fit by a given model were actually generated by another
model. Assuming equal prior probabilities across models, the inversion
matrix can be obtained simply by re-normalizing the confusion matrix
across columns.
...
Busemeyer J. R. Oxford University Press.
