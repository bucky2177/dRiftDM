test_that("testing DMC", {
  ...
  a_dmc_model = set_model_prms(a_dmc_model, c(3.952847,
  ...
  sim_data = DMCfun::dmcSim(drc = 0.5059644,
                            bnds = 75.89466,
                            amp = 12.64911,
                            tau = 40,
                            resMean = 300,
  ...
  expect_true(
    all(abs(
      sim_data$delta$meanComp -
        dmc_quants$Quant_Corr[dmc_quants$Cond == "comp"]*1000)< 10)
  )

  expect_true(
    all(abs(
      sim_data$delta$meanIncomp -
        dmc_quants$Quant_Corr[dmc_quants$Cond == "incomp"]*1000)< 10)
  )

  expect_true(
    all(abs(
      sim_data$caf$accPerComp - dmc_cafs$P_Corr[dmc_cafs$Cond == "comp"]) < .01)
  )

  expect_true(
    all(abs(
      sim_data$caf$accPerIncomp[2:5] -
        dmc_cafs$P_Corr[dmc_cafs$Cond == "incomp"][2:5]) < .01)
  )

  expect_true(
    all(abs(
      sim_data$caf$accPerIncomp[1] -
        dmc_cafs$P_Corr[dmc_cafs$Cond == "incomp"][1]) < .035)
  ...
  expect_true(
    all(abs(
      sim_data$caf$accPerIncomp[1] -
        dmc_cafs$P_Corr[dmc_cafs$Cond == "incomp"][1]) < .03)
...
test_that("ratcliff_simple works as expected", {
  ...
  sim_data = DMCfun::dmcSim(drc = 0.3794733,
                            bnds = 75.89466,
                            amp = 0,
                            tau = 40,
                            resMean = 300,
  ...
  expect_true(
    all(abs(
      sim_data$delta$meanComp -
        r_quants$Quant_Corr[r_quants$Cond == "null"]*1000) < 10)
  )


  expect_true(
    all(abs(
      sim_data$caf$accPerComp - r_cafs$P_Corr[r_cafs$Cond == "null"]) < .01)
  )
