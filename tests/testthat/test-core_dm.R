...
test_that("creating a drift_dm object", {
  ...
  expect_identical(a_model$flex_prms_obj,
...
test_that("check_raw_data specific input checks", {
  ...
  expect_warning(
    check_raw_data(temp_data, def_b_coding$column, def_b_coding$u_name_value,
...
test_that("comp_vals -> return vectors fail as expected", {
  ...
  a_model = drift_dm(prms_model = c(a = 2, b = 3), conds = c("foo", "bar"),
  ...
  expect_error(re_evaluate_model(temp),
...
test_that("simulate_traces -> works as expected", {
  ...
  pdf_x <- dunif(xx, min = -coef(a_model)["range_start"]/2,
  ...
  expect_equal(names(attributes(out$comp)),
...
test_that("simulate_data.drift_dm -> single data set works as expected", {
  ...
  b_coding(a_model) <- list(column = "col",u_name_value = c(a = 0),
                            l_name_value = c(b = -1))
  sim_data <- simulate_data(a_model, n = 100000, seed = 1)
  test = check_raw_data(sim_data, b_coding_column = "col", u_value = 0,
                        l_value = -1) # only to check for unexpected errors
...
test_that("simulate_data.drift_dm -> multiple data sets works as expected", {
  ...
  df_prms = data.frame(b = c(0.5, 0.6),
                       ID = c("foo", 2),
                       muc = c(3,4),
                       non_dec = c(0.3, 0.4))

  # generate some data using df_prms
  sim_data <- simulate_data(a_model, n = 1000, df_prms = df_prms,
                            seed = 1)
  ...
  all_data <- simulate_data(a_model, n = 100, lower = lower, upper = upper,
                            k = 2, seed = 1)
...
test_that("simulate_data.drift_dm -> input checks", {
  ...
  expect_error(simulate_data(a_model, n = c(2,3,4)),
               "must have as many entries")
  expect_error(simulate_data(a_model, n = c(comp = 2, ho = 2)),
               "specify names")

  expect_error(simulate_data(a_model, n = 10, seed = c("1", "2")),
               "single numeric")
  ...
  expect_error(simulate_data(a_model, 10, lower = c(1,2,3), upper = c(1,2,3),
                             df_prms = data.frame()),
               "lower/upper OR df_prms, not both")
  expect_error(simulate_data(a_model, 10, k = 2),
               "lower/upper OR df_prms")

  expect_error(simulate_data(a_model, 10, k = 2, df_prms = list()),
