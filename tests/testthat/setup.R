...
ssp_dummy <- ssp_dm(obs_data = dRiftDM::ssp_synth_data)
ssp_dummy <- re_evaluate_model(ssp_dummy)

dmc_dummy <- dmc_dm(obs_data = dRiftDM::dmc_synth_data)
dmc_dummy <- re_evaluate_model(dmc_dummy)

ratcliff_dummy <- ratcliff_dm(obs_data = dRiftDM::ratcliff_synth_data)
