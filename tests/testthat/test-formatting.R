...
test_that("summary works as expected", {
  ...
  expect_identical(summary_model$nPrms,
                   c("total" = length(a_dmc_model$prms_model),
  ...
  expect_identical(summary_model$fit_stats,
                   c("log(like)" = a_dmc_model$log_like_val,
                     "aic" = a_dmc_model$ic_vals[["AIC"]],
                     "bic" = a_dmc_model$ic_vals[["BIC"]]))

  summary_data = cbind(summary(a_dmc_model$obs_data$rts_corr$comp),
  ...
  rownames(summary_data) = c("correct comp", "correct incomp",
                             "error comp", "error incomp")
  summary_data = cbind(summary_data,
                       c(length(a_dmc_model$obs_data$rts_corr$comp),
