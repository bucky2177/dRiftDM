test_that("snapshot stats_list plots", {
  ...
  vdiffr::expect_doppelganger(title = "cafs_quantiles_delta",
                              fig = plot_all())
...
test_that("snapshot the traces plot", {
  ...
  plot_all_traces <- function() {
    plot(traces_obj, col = c("green", "red"),
         col_b = "blue", xlim = c(-0.1, 0.4), ylim = c(-0.7, 0.7),
         xlab = "foo", ylab = "bar", lty = 2, legend = c("hoo", "ha"),
         legend_pos = "bottomright")
  }
  vdiffr::expect_doppelganger(title = "all_traces",
                              fig = plot_all_traces())

  plot_one_trace = function() {
    plot(traces_obj[[1]], col = c("green"),
         col_b = "blue", xlim = c(-0.1, 0.4), ylim = c(-0.7, 0.7),
         xlab = "foo", ylab = "bar", lty = 2)
  }

  vdiffr::expect_doppelganger(title = "one_traces",
                              fig = plot_one_trace())
...
test_that("snapshot for coefs.dm plot", {
  ...
  plot_hists1 <- function() {
    hist(all_prms_1, main = c("foo", "bar", "ho"),
         colors = "blue", xlab = "test")
  }
  vdiffr::expect_doppelganger(title = "unique_prms_hist",
                              fig = plot_hists1())

  plot_hists2 <- function() {
    hist(all_prms_2, alpha = 0.2, colors = c("green", "red", "blue"))
  }
  vdiffr::expect_doppelganger(title = "cond_prms_hist",
                              fig = plot_hists2())
...
test_that("snapshot for plot.drift_dm", {
  ...
  plot_model1 <- function() {
    plot(a_model, conds = "comp", color = "green", xlim = c(0, 0.5),
         legend = "foo", legend_pos = "bottomright")
  }
  vdiffr::expect_doppelganger(title = "plot_model_one_cond",
                              fig = plot_model1())
  ...
  vdiffr::expect_doppelganger(title = "plot_model",
                              fig = plot_model2())
