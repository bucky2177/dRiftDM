...
test_that("estimate_dm -> sep_c works as expected", {
  ...
  expect_message(
    expect_message(
      expect_message(
        tmp2 <- estimate_dm(
          drift_dm_obj = tmp2, obs_data = data, lower = l_u$lower,
          upper = l_u$upper, messaging = FALSE,
          control = list(VTR = 0, trace = FALSE)
        ), "Starting optimizer"
  ...
  expect_snapshot(
    all_fits <- estimate_dm(
      ...
      messaging = TRUE, verbose = 0, progress = 0,
      control = list(VTR = 0, trace = FALSE)
    )
  )
  expect_identical(
...
test_that("estimate_dm -> sep_b works as expected", {
  ...
  expect_snapshot(
    all_fits <- estimate_dm(
      drift_dm_obj = tmp, obs_data = data, approach = "sep_b", lower = l_u$lower,
      messaging = TRUE, verbose = 0, burn_in = 1, progress = 0, samples = 1,
...
test_that("estimate_dm -> hier_b runs as expected", {
  ...
  expect_error(
    estimate_dm(
      drift_dm_obj = model, obs_data = tmp, approach = "hier_b",
      messaging = FALSE
    ), "No 'ID' column"
  )
...
test_that("estimate_dm throws warning/errors for unreasonable input", {
  ...
  expect_warning(
    expect_warning(
      estimate_dm(
        drift_dm_obj = model, obs_data = tmp, control = list(maxit = 1),
        progress = 0, verbose = 0, messaging = FALSE
  ...
  expect_snapshot(
    out <- estimate_dm(
      drift_dm_obj = model, obs_data = data, approach = "sep_b",
      optimizer = "Nelder-Mead", samples = 1, burn_in = 1, n_chains = 4,
      progress = 0, verbose = 0, messaging = TRUE
    )
  )
...
test_that("estimate_classical -> start_vals are applied and correctly mapped", {
  ...
  expect_snapshot(
    suppressWarnings(
      estimate_classical(
        model, verbose = 2, optimizer = "Nelder-Mead",
  ...
  expect_snapshot(
    suppressWarnings(
      res <- estimate_classical(
        model, verbose = 2, optimizer = "Nelder-Mead",
        control = list(maxit = 1),
        start_vals = start_vals, round_digits = 2
      )
    )
  )
  ...
  suppressWarnings(
    res <- estimate_classical(
      model, verbose = 0, optimizer = "Nelder-Mead", control = list(maxit = 1),
      start_vals = start_vals, return_runs = TRUE
    )
  )
...
test_that("estimate_classical -> optimizer is dispatched correctly", {
  ...
  expect_error(
    estimate_classical(
      model, verbose = 0, optimizer = "Nelder-Mead", start_vals = NA
    ), "'start_vals'"
  )
  ...
  expect_error(
    estimate_classical(
      model, verbose = 0, optimizer = "Nelder-Mead", start_vals = start_vals,
      return_runs = NA, control = list(maxit = 1)
    ), "'return_runs'"
  )
  ...
  expect_warning(
    estimate_classical(
      model, verbose = 0, optimizer = "DEoptim", start_vals = start_vals,
      lower = l_u$lower, upper = l_u$upper, control = list(VTR = 5000, trace = TRUE)
...
test_that("estimate_clasical -> correctly dispatches to stats::optim", {
  ...
  mocked_optim = function(...) {
    ...
    list(par = dots$par, counts = 1L, convergence = 0L,
         message = dots$method)
  }
  ...
  with_mocked_bindings({
    res <- estimate_classical(
      drift_dm_obj = model, optimizer = "Nelder-Mead", verbose = 0
    )
  }, optim = mocked_optim, .package = "stats")
  ...
  with_mocked_bindings({
    res <- estimate_classical(
      drift_dm_obj = model, optimizer = "BFGS", verbose = 0
    )
  }, optim = mocked_optim, .package = "stats")
  ...
  with_mocked_bindings({
    res <- estimate_classical(
      drift_dm_obj = model, optimizer = "L-BFGS-B", verbose = 0,
...
test_that("estimate_clasical -> correctly dispatches to dfoptim::dfoptim", {
  ...
  with_mocked_bindings({
    res <- estimate_classical(
      drift_dm_obj = model, optimizer = "nmkb", verbose = 0,
      lower = l_u$lower, upper = l_u$upper
    )
  }, nmkb = mocked_nmkb, .package = "dfoptim")
...
test_that("estimate_classical_wrapper -> start_vals correctly passed forward", {
  ...
  expect_snapshot(
    suppressWarnings(
      estimate_classical_wrapper(
        model, obs_data_ids = data, verbose = 2, optimizer = "Nelder-Mead",
        control = list(maxit = 1), start_vals = start_vals, progress = 0
      )
    )
  )
...
test_that("estimate_classical_wrapper -> multiple core usage works", {
  ...
  suppressWarnings(
    res_2 <- estimate_classical_wrapper(
      model, obs_data_ids = data, optimizer = "Nelder-Mead", n_cores = 2,
      control = list(maxit = 20), progress = 0
    )
  )
  ...
  suppressWarnings(
    res_1 <- estimate_classical_wrapper(
      model, obs_data_ids = data, optimizer = "Nelder-Mead", n_cores = 1,
      control = list(maxit = 20), progress = 0
    )
  )
  ...
  expect_snapshot(
    suppressWarnings(
      res_3 <- estimate_classical_wrapper(
        model, obs_data_ids = data, optimizer = "DEoptim", n_cores = 2,
        lower = l_u$lower, upper = l_u$upper, progress = 0,
        control = list(VTR = -300, trace = TRUE), parallelization_strategy = 2,
        seed = 1
      )
    )
  )
  ...
  expect_snapshot({
    suppressWarnings(
      res_4 <- estimate_classical_wrapper(
        model, obs_data_ids = data, optimizer = "DEoptim", n_cores = 2,
        lower = l_u$lower, upper = l_u$upper, progress = 0,
        control = list(VTR = -300, trace = TRUE), parallelization_strategy = 1,
        seed = 1
      )
    )
    coef(res_4)
...
test_that("estimate_classical_wrapper -> input error messages", {
  ...
  expect_error(
    estimate_classical_wrapper(
      drift_dm_obj = model, obs_data_ids = data, optimizer = "DEoptim",
  ...
  expect_error(
    estimate_classical_wrapper(
      drift_dm_obj = model, obs_data_ids = data, optimizer = "Nelder-Mead",
      progress = -1
    ),
    "'progress' must be >= 0"
  )
