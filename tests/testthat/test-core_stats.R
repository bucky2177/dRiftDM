test_that("calc_caf works as expected", {
  ...
  dat = data.frame(RT = c(rts_1, rts_2),
                   Error = c(errs_1, errs_2),
                   Cond = rep(c("null", "foo"), each = 12))

  # add to model
  dummy_model = set_obs_data(drift_dm_obj = dummy_model,
                             obs_data = dat)
  ...
  expect_identical(calc_cafs(drift_dm_obj = dummy_model, type = "obs",
                             n_bins = 6),
                   calc_cafs_obs(drift_dm_obj = dummy_model, n_bins = 6))

  # input checks
  expect_error(calc_cafs(drift_dm_obj = dummy_model, n_bins = 1),
               "larger than 1")
  expect_error(calc_cafs(drift_dm_obj = dummy_model, n_bins = NA),
               "single numeric")
  expect_error(calc_cafs(drift_dm_obj = dummy_model, type = "foo"),
               "should be one of")
...
test_that("calc_quantiles works as expected", {
  ...
  quants_obs = calc_quantiles(drift_dm_obj = dummy_model, type = "obs",
  ...
  # both
  quants_both = calc_quantiles(dummy_model, type = "both",
                               probs = seq(0.2, 0.8, 0.1))
  ...
  # input checks
  expect_error(calc_quantiles(dummy_model, type = "foo"),
               "should be one of")
  expect_error(calc_quantiles(dummy_model, probs = NA),
               "numeric vector")
  expect_error(calc_quantiles(dummy_model, probs = numeric()),
               "numeric vector")
  expect_error(calc_quantiles(dummy_model, probs = c(0, 0.1)),
               "must be in the range")
  expect_error(calc_quantiles(dummy_model, probs = c(0.1, 1)),
               "must be in the range")
