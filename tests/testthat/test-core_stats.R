...
test_that("calc_cafs -> input checks", {
  ...
  # input checks
  expect_error(
    calc_cafs(pdf_u = NULL, pdf_l = NULL, rts_u = NULL, rts_l = NULL,
  ...
  expect_error(
    calc_cafs(pdf_u = c(0, 1, 0), pdf_l = NULL, rts_u = NULL, rts_l = NULL,
              one_cond = "foo", b_coding = b_coding),
    "both NULL or not"
  )

  expect_error(
    calc_cafs(pdf_u = NULL, pdf_l = NULL, rts_u = c(0, 1, 0), rts_l = NULL,
              one_cond = "foo", b_coding = b_coding),
    "both NULL or not"
  )
...
test_that("calc_quantiles -> observed and predicted works as expected", {
  ...
  obs_data(dummy_model) <- dat
  quants <- calc_stats(dummy_model, type = "quantiles",
  ...
  quants_obs_new_b = calc_stats(dat, type = "quantiles",
                                probs = seq(0.2, 0.8, 0.1),
                                conds = "incomp",
                                b_coding = list(column = "Foo",
...
test_that("quantiles -> validate and aggregate work as expected", {
  ...
  test = aggregate(cbind(Quant_corr, Quant_err) ~ Prob + Cond + Source,
                   obs_quants, mean, na.rm = T, na.action = na.pass)
...
test_that("calc_quantiles -> input checks", {
  ...
  # input checks
  expect_error(
    calc_quantiles(pdf_u = NULL, pdf_l = NULL, t_vec = t,
  ...
  expect_error(
    calc_quantiles(pdf_u = c(0,1, 2), pdf_l = NULL, t_vec = t,
                   rts_u = NULL, rts_l = NULL, one_cond = "foo",
                   b_coding = b_coding),
    "both NULL or not"
  )
...
test_that("calc_delta_funs -> observed and predicted works as expected", {
  ...
  new_flex_prms = flex_prms(object = prms,
  ...
  delta_dat <- calc_stats(
    a_model, type = "delta_fun", minuends = "incomp", subtrahends = "comp"
  )
  expect_equal(
  ...
  obs_delta_dat <- calc_stats(
    data, type = "delta_fun", minuends = "incomp", subtrahends = "comp"
  )
...
test_that("delta_funs -> validate and aggregate work as expected", {
  ...
  data_id = dRiftDM::ulrich_flanker_data
  obs_delta = calc_stats(data_id, type = "delta_funs", minuends = "incomp",
                          subtrahends = "comp")


  test = aggregate(cbind(Quant_corr_comp, Delta_incomp_comp) ~ Prob + Source,
                   obs_delta, mean, na.rm = T, na.action = na.pass)
  test = test[c("Source", "Prob", "Quant_corr_comp", "Delta_incomp_comp")]

  # test what is returned
  avg_delta = calc_stats(data_id,  type = "delta_funs", minuends = "incomp",
...
test_that("calc_delta_funs -> input checks", {
  ...
  # input checks
  expect_error(
    calc_delta_funs(quantiles_dat = quantiles,
  ...
  expect_error(
    calc_delta_funs(quantiles_dat = temp,
                    minuends = "comp",
                    subtrahends = "incomp",
...
test_that("calc_fit_stats -> works as expected", {
  ...
  exp_stats = data.frame(Log_Like = for_test_log_like,
                         AIC = for_test_AIC,
                         BIC = for_test_BIC)
...
test_that("fits_ids_dm calc_stats works as expected", {
  ...
  expect_equal(all_2[c("Log_Like", "AIC", "BIC")],
  ...
  expect_equal(class(all_2), class(sep_2))
  expect_equal(all_2[c("Source", "Cond", "Bin", "P_corr")],
