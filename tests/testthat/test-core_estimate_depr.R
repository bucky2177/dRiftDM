test_that("input checks estimate_model", {
  ...
  expect_warning(w_estimate_model(a_model,
                                lower = c(1, 0.2, 0.1),
                                upper = c(7, 0.8, 0.6)
  ), "No data set")
  return_val <- suppressWarnings(w_estimate_model(a_model,
                                                lower = c(1, 0.2, 0.1),
                                                upper = c(7, 0.8, 0.6)
  ))
  ...
  a_model <- ratcliff_dummy
  expect_error(w_estimate_model(a_model,
                              lower = c(0.2, 0.1),
                              upper = c(7, 0.8, 0.6)
 ...
 return_val <- suppressWarnings(w_estimate_model(a_model,
  ...
  expect_error(
    w_estimate_model(a_model,
                   lower = c(1, 0.2, 0.1),
                   upper = c(7, 0.8, 0.6), use_de_optim = FALSE, use_nmkb = TRUE,
                   nmkb_control = "a"
    ),
    "control"
  )
  ...
  expect_error(
    w_estimate_model(
      a_model,
      lower = list(
        default_values = c(muc = 1, b = 0.3, non_dec = 0.4), null = c(foo = 3)
        ), upper = c(muc = 3, b = 5, non_dec = 0.4)
    ),
