...
test_that("modify_flex_prms -> all instructions work as expected", {
  ...
  # vary
  a_flex_prms_obj = modify_flex_prms(object = a_flex_prms_obj,
                                     instr = "b ~ bar
                                              c ~ ")
  expect_equal(unlist(a_flex_prms_obj$linear_internal_list$a),
               c(foo = 1, bar = 1, uff = 1))
  expect_equal(unlist(a_flex_prms_obj$linear_internal_list$b),
               c(foo = 2, bar = 3, uff = 2))
  expect_equal(unlist(a_flex_prms_obj$linear_internal_list$c),
               c(foo = 4, bar = 5, uff = 6))
  ...
  a_flex_prms_obj = modify_flex_prms(object = a_flex_prms_obj,
                                     instr = "a + c ~ => 0.3
                                              b ~ bar + uff => 0.4")
  expect_equal(a_flex_prms_obj$prms_matrix[,1],
               c(foo = 0.3, bar = 0.3, uff = 0.3))
  expect_equal(a_flex_prms_obj$prms_matrix[,2],
               c(foo = 3, bar = 0.4, uff = 0.4))
  expect_equal(a_flex_prms_obj$prms_matrix[,3],
               c(foo = 0.3, bar = 0.3, uff = 0.3))
  ...
  expect_equal(a_flex_prms_obj$cust_prms$values$d,
               c(foo = -0.4, bar = -0.4, uff = 0.7))
  expect_equal(a_flex_prms_obj$cust_prms$values$e,
...
test_that("validate_flex_prms -> errs as expected", {
  ...
  expect_error(validate_flex_prms(temp),
               "can not be adressed")
...
test_that("modify_flex_prms -> drift_dm", {
  ...
  expect_equal(a_model$flex_prms_obj$prms_matrix[,"alpha"],
               c(comp = 5, incomp = 5))
