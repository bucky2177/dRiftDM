...
test_that("modify_flex_prms -> all instructions work as expected", {
  ...
  expect_equal(a_flex_prms_obj$linear_internal_list$a, list(foo = 1, bar = 0))

  expect_equal(a_flex_prms_obj$linear_internal_list$b, list(foo = 2, bar = 0))
  expect_equal(a_flex_prms_obj$linear_internal_list$c, list(foo = 3, bar = 0))
})
...
test_that("messages and warnings", {
  ...
  expect_no_message(modify_flex_prms(
    a_flex_prms_obj,
    "a ~ bar => 0.5",
    messaging = F
  ))
