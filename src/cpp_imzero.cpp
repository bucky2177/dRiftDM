  ...
  int cpp_imzero(NumericVector& pdf_u,
    ...
    for (int ti=1;ti<=nt;++ti)
    {
      ...
      for (int j=1;j<ti;++j)
      {
        const double F11 = ff(t_vec[ti], t_vec[j], b_vals[ti], b_vals[j],mu_int_vals[ti], mu_int_vals[j],sqrt_sigma_vals[ti-j+1],2*sigma*sigma, dt_b_vals[ti],mu_vals[ti]);
        const double F12 = ff(t_vec[ti], t_vec[j], b_vals[ti],-b_vals[j],mu_int_vals[ti], mu_int_vals[j],sqrt_sigma_vals[ti-j+1],2*sigma*sigma, dt_b_vals[ti],mu_vals[ti]);
        const double F21 = ff(t_vec[ti], t_vec[j],-b_vals[ti], b_vals[j],mu_int_vals[ti], mu_int_vals[j],sqrt_sigma_vals[ti-j+1],2*sigma*sigma,-dt_b_vals[ti],mu_vals[ti]);
        const double F22 = ff(t_vec[ti], t_vec[j],-b_vals[ti],-b_vals[j],mu_int_vals[ti], mu_int_vals[j],sqrt_sigma_vals[ti-j+1],2*sigma*sigma,-dt_b_vals[ti],mu_vals[ti]);
