...
double ff(double t, double t0,
          double b1, double b2, 
          double mu_int1, double mu_int2, 
          double sqrt_sigma, double sigma_sq, 
          double dt_b, double mu)
{
  double tmp  = b1-b2-mu_int1+mu_int2;
  double ttmp = t-t0;
  double multipl = 1./sqrt_sigma;
  double expv = exp(-tmp*tmp/(sigma_sq*ttmp));
  double denom = (dt_b-mu-tmp/ttmp);
  return multipl * expv * 0.5 * denom;
}

double psi(double b, double t, double dt_b, double mu, double mu_int, double mu_int0, double sqrt_sigma)
{
  double n1 = 1.0/sqrt_sigma * exp( - pow(b-mu_int+mu_int0,2.0) / (sqrt_sigma*sqrt_sigma/M_PI));
  double n2 = dt_b - mu - (b-mu_int+mu_int0)/t;
  return 0.5*n1*n2;
}
...
int cpp_imzero(NumericVector& pdf_u,
  ...
  for (int ti=1;ti<=nt;++ti)
  {
    ...
    for (int j=1;j<ti;++j)
    {
      double F11 = ff(t_vec[ti], t_vec[j], b_vals[ti], b_vals[j],mu_int_vals[ti], mu_int_vals[j],sqrt_sigma_vals[ti-j+1],2*sigma*sigma, dt_b_vals[ti],mu_vals[ti]);
      double F12 = ff(t_vec[ti], t_vec[j], b_vals[ti],-b_vals[j],mu_int_vals[ti], mu_int_vals[j],sqrt_sigma_vals[ti-j+1],2*sigma*sigma, dt_b_vals[ti],mu_vals[ti]);
      double F21 = ff(t_vec[ti], t_vec[j],-b_vals[ti], b_vals[j],mu_int_vals[ti], mu_int_vals[j],sqrt_sigma_vals[ti-j+1],2*sigma*sigma,-dt_b_vals[ti],mu_vals[ti]);
      double F22 = ff(t_vec[ti], t_vec[j],-b_vals[ti],-b_vals[j],mu_int_vals[ti], mu_int_vals[j],sqrt_sigma_vals[ti-j+1],2*sigma*sigma,-dt_b_vals[ti],mu_vals[ti]);
