...
#'
calc_pdfs <- function(drift_dm_obj, x_vec, t_vec, prms_solve, solver) {
  UseMethod("calc_pdfs")
}

#' @export
calc_pdfs.ratcliff_dm <- function(drift_dm_obj, x_vec, t_vec, prms_solve, solver) {
  ...
  pdfs = sapply(conds, \(one_cond){
    pdf_u = rowSums(sapply(all_pdfs, \(x){
      x[[one_cond]]$pdf_u
    }))
    
    pdf_l = rowSums(sapply(all_pdfs, \(x){
      x[[one_cond]]$pdf_l
    }))
...
#' @export
calc_pdfs.drift_dm <- function(drift_dm_obj, x_vec, t_vec, prms_solve, solver, 
                               wgt = 1) {
  ...
  pdfs <- sapply(conds, function(one_cond) {
    ...
    pdfs_one_cond = list(pdf_u = pdf_u * wgt, pdf_l = pdf_l * wgt)
