...
- Bayesian hierarchical estimation is now fully reproducable when using
...
- We reverted back to adding the value `0.001` to `t_vec` when
  evaluating DMC’s drift rate for `a!=2` (after noticing solver crashes
  with the former value of `1e-5`).

- It is now possible to pass a value via
  ...
  this controls the value that is added to `t_vec` when evaluating DMC’s
  drift rate for `a!=2`.
