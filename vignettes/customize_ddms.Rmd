...
flex_prms(ddm)
```
While this may be a bit annoying in some cases, it actually makes sense because there is no way for dRifDM to know how the new conditions relate to the old ones. Consequently, we would now have to modify the underlying `flex_prms` object again to suit our needs. For example, we could restore the previous behavior for `A` in the `incomp` condition, while setting `A` to zero for the new `neutral` condition. We might also want to keep `a` again fixed for all conditions:
...
- `mu_fun()` and `mu_int_fun()` are the drift rate and its integral. The integral is required as an entry, but is only evaluated if the user chooses the non-default method "im_zero" to derive model predictions.
<!--@Vali: Gibt es einen Unterscied zwischen ...are the drift rate.. und dann immer ..returns the...?-->
...
The structure of each of these component functions is generally the same. The drift rate, boundary, and non-decision time functions (i.e., mu_fun, mu_int_fun, b_fun, dt_b_fun, and nt_fun<!--@Vali: in Klammern alle in `...` ?-->) must have the following declaration: 
...
- `prms_model` is a named numeric vector and is identical to a row of the **Parameter Matrix** <!--@Vali: welche ist das?-->of a model.
...
tail(t_vec)
```
- `one_cond` is the label of the current condition for which the model is being evaluated (i.e., a row name of the **Parameter Matrix**).<!--@Vali: hier auch-->
...
$$\mu(t) = \left\{ 
  ...
  \end{array}
\right.$$
Thus, in compatible conditions, the drift rate at each time step is the sum of the two drift rates $muc$ and $mua$, while in incompatible conditions it is their difference. 
...
Then we write a function that assembles the custom model:
<!--@Vali: im COde hast du manchmal Werte ohen Führungsnull und manchmal mit, vielleicht
noch einheitlih machen?-->

```{r}
cust_model <- function() {
  # define all parameters and conditions
  prms_model <- c(
    b0 = 0.6,      # parameters for the custom boundary
    kappa = 0.5,
    t05 = 0.15, 
    muc = 4,       # parameter for a time-independent drift rate "muc"
    non_dec = .2   # parameter for a non-decision time "non_dec"
...
## Example: Custom Non-Decision Time
<!--@Vali: Man könnte die Examples noch durchnummerieren? Und dann oben einmal erwänen, was alles kommt und eben verweisen? "The following section provides examples for a custom drift rate (Example 1), ..."?-->
